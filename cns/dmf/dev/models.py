...
class Model(object):
    ...
    def __init__(self, C, G=0, GI=0, bold='obata', params=None, verbose=True,
        ...
        self._I_ext_inh = np.repeat(model_params['I_ext'], self._nc)
        ...
        self._hierarchy_gradient_rev = 0.0
        ...
        if myelin is not None:
            if gradient == 'forward':
                ...
                myelin_range_rev = self._myelin - np.min(self._myelin)  # np.ptp(self._myelin)
                self._hierarchy_gradient_rev = myelin_range_rev / np.max(myelin_range_rev)
            else:
                myelin_range = self._myelin - np.min(self._myelin)  # np.ptp(self._myelin)
                self._hierarchy_gradient = myelin_range / np.max(myelin_range)

                myelin_range_rev = np.ptp(self._myelin)
                self._hierarchy_gradient_rev = (-(self._myelin - np.max(self._myelin)) / myelin_range_rev)
        ...
        if thickness is not None:
            ...
            thickness_range_rev = np.ptp(self._thickness)
            self._hierarchy_gradient_rev = (-(self._thickness - np.max(self._thickness)) / thickness_range_rev)
    ...
    def _inh_curr_fixed_pts(self, I):
        ...
        return self._I0_I + self._K_EI.dot(self._S_E_ss) + self._I_ext_inh - \
    ...
    def _apply_hierarchy(self, a, b):
        ...
        if b < 0.0:
            return a + np.abs(b) * self._hierarchy_gradient_rev
        else:
            return a + b * self._hierarchy_gradient


    def _solve_lyapunov(self, jacobian, evals, L, Q, bold=False, builtin=False, full_cov=False):
        if builtin:
            ...
            if bold:
                ...
                if full_cov:
                    ...
                    self._full_cov = L.dot(M.dot(L_dagger))
            else:
                L_dagger = np.conj(L).T
                cov = L.dot(M.dot(L_dagger)).real
            return cov

    def _linearized_cov(self, use_lyapunov=False, BOLD=False, full_cov=False):
        ...
        if self._unstable:
            ...
            if BOLD:
                ...
                self._cov_bold = self._solve_lyapunov(self._jacobian_bold, evals, evects, self._Q_bold, bold=True,
                                                      builtin=use_lyapunov, full_cov=full_cov)
    ...
    def power_spectrum_test(self, freqs, pop='E', tau=0.0):
        """Returns the power in each cortical area at the given frequencies
        for the specified neuronal population ('E' or 'I'). """
        if self._jacobian is None: self._update_matrices()
        Id = np.identity(self._nc * 2)
    ...
    def coherence(self, freqs, pop='E', tau=0.0):
        ...
        density = self.power_spectrum(freqs, pop=pop, tau=tau)
        if density.ndim > 2: density = density.mean(2)
        power = np.diag(density)
        power_diag = np.tile(power, (self._nc, 1))
        #coh = (density ** 2) / (power_diag * power_diag.T)
        coh = (density) / np.sqrt(power_diag * power_diag.T)
        return coh, power
    ...
    def timescales(self, freqs=np.arange(-5, 5, 0.05), pop='E'):
        ...
        power_spec = np.empty((self._nc, len(freqs)))
        for i, f in enumerate(freqs):
        ...
        def optimal_params(f, power):
            ...
            params, pcov = curve_fit(lorentzian, f, power, p0=[10., 1e-6], maxfev=1000)
            print params[0]
            return params, pcov

        # Compute timescales
        tscales = np.empty(self._nc)
    ...
    def moments_method(self, recompute_FIC=True, BOLD=False, use_lyapunov=False, full_cov=False):
        self._update_matrices(compute_FIC=recompute_FIC)
        self._linearized_cov(use_lyapunov=use_lyapunov, BOLD=BOLD, full_cov=full_cov)
        self.reset_state()
