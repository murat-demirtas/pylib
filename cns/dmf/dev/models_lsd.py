...
class Model(object):
    ...
    def __init__(self, C, G=0, GI=0, bold='obata', params=None, verbose=True,
                 myelin = None, serotonin = None, grad_scale=(1.0, 0.), rec_scale=(0.15, 0.), ee_scale=(0.15, 0.), ei_scale=(0.15, 0.),
        ...
        #self._a_E = np.repeat(model_params['a_E'], self._nc)
        ...
        self._aa = np.ones(self._nc)

        if myelin is not None:
            myelin_range = np.ptp(self._myelin)
            self._myelin_gradient = (-(self._myelin - np.max(self._myelin)) / myelin_range)
        ...
        if serotonin is not None:
            serotonin_range = np.ptp(self._serotonin)
            self._serotonin_gradient = (-(self._serotonin - np.max(self._serotonin)) / serotonin_range)
    ...
    def _phi(self):
        ...
        II = sym.symbols('II')
        #a = sym.symbols('a')
        #phi_E = (a * IE - self._b_E) / (1. - sym.exp(-self._d_E * (a * IE - self._b_E)))
    ...
    def _update_matrices(self, compute_FIC=True):
        ...
        # Connectivity
        self._K_EE = (self._J_NMDA_rec * self._w_EE * eye) + (self._G * self._J_NMDA_EE * self._SC)
        ...
        dr_E = self.dphi_E(self._aa*self._I_E_ss) * eye
        dr_I = self.dphi_I(self._I_I_ss) * eye

        self._r_E_ss = self.phi_E(self._aa*self._I_E_ss)
    ...
    def _apply_hierarchy(self, a, b):
        ...
        return a + b * self._myelin_gradient

    def _apply_gradient(self, a, b):
        """Parametrize excitatory self-recurrence strength
                according to cortical myelination values. """
        return a + b * self._serotonin_gradient
    ...
    def _linearized_cov(self, use_lyapunov=False, BOLD=False):
        ...
        if self._unstable:
            ...
            if BOLD:
                ...
                self._cov = self._full_cov[:2 * self._nc, :2 * self._nc]
    ...
    def coherence(self, freqs, pop='E', tau=0.0):
        #density = np.abs(self.power_spectrum(freqs, pop=pop))
        #density = np.abs(self.power_spectrum(freqs, pop=pop, tau=tau))
    ...
    def coherence_tau(self, freqs, pop='E', tau=0.0):
        density = np.abs(self.power_spectrum(freqs, pop=pop, tau=0.0))
        if density.ndim > 2: density = density.sum(2)
        power = np.diag(density)
        power_diag = np.tile(power, (self._nc, 1))
        density_tau = np.abs(self.power_spectrum(freqs, pop=pop, tau=tau))
        if density_tau.ndim > 2: density_tau = density_tau.sum(2)
        #power = np.diag(density_tau)
        #power_diag = np.tile(power, (self._nc, 1))
        coh = (density_tau ** 2) / (power_diag * power_diag.T)
        return coh
    ...
    def timescales(self, freqs=np.arange(-5, 5, 0.05), pop='E'):
        ...
        power_spec_density = self.power_spectrum(freqs, pop=pop)
    ...
    def cov_tau(self, tau=1.0):
        return self._cov.dot(expm(self._jacobian.T * tau))[:self._nc, :self._nc]

    def corr_tau(self, tau=1.0):
        return self.cov_tau(tau=tau) / np.sqrt(np.outer(self.var, self.var))

    def cov_bold_tau(self, tau=1.0):
        return np.dot(np.dot(self.hemo.B, self._full_cov.dot(expm(self._jacobian_bold.T * tau))), self.hemo.B.conj().T)
