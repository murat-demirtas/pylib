...
class HopfModel():
    ...
    def _apply_hierarchy(self, a, b):
        ...
        if isinstance(b, float):
            ...
            if self._hmap.shape[0] == (len(b) - 1):
                updated_map = self._hmap.T.dot(b[:-1])
                return a + self._normalize_map(updated_map) * b[-1]
            elif self._hmap.shape[0] == len(b):
                return a + self._hmap.T.dot(b)            
            else:
                assert(self._hmap.shape[0] == len(b))

    def _normalize_map(self, hmap):
        hmap_norm = hmap - np.min(hmap)
        return hmap_norm / np.max(hmap_norm)


    def moments_method(self, use_lyapunov=False):
