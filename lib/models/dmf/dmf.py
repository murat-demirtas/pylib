...
class Model(object):
    ...
    def __init__(self, sc, g=0, norm_sc=True, hmap = None, gmap = None,
        ...
        if self._raw_hmap is not None:
            if np.ndim(self._raw_hmap) == 1 or any(np.array(self._raw_hmap.shape) == 1):
                hmap_norm = self._raw_hmap - np.min(self._raw_hmap)
                self._hmap = hmap_norm / np.max(hmap_norm)
            else:
                self._hmap = self._raw_hmap.mean(0)

        if self._raw_gain_map is not None:
            if np.ndim(self._raw_gain_map) == 1 or any(np.array(self._raw_gain_map.shape) == 1):
                hmap_norm = self._raw_gain_map - np.min(self._raw_gain_map)
                self._gain_map = hmap_norm / np.max(hmap_norm)
            else:
                self._gain_map = self._raw_gain_map.mean(0)

        # Set SC normalization
        self._sc_norm_e = 1.0
        self._sc_norm_i = 1.0
        if norm_sc:
            sc_norm_e = 1. / self._SC.sum(1)
            self._sc_norm_e = np.tile(sc_norm_e, (self._nc, 1)).T

            sc_norm_i = 1. / self._SC.sum(1)
    ...
    def set_jacobian(self, compute_fic=True):
        ...
        self._K_EE = (self._w_EE * eye) + (self._G * self._J_NMDA * self._sc_norm_e * self._SC)
        self._K_EI = (self._w_EI * eye) + (self._Gi * self._J_NMDA * self._sc_norm_i * self._SC)
        ...
        if np.any(self._w_IE < 0):
            #print("Warning: Feedback inhibition leads to negative values...")
            self._unstable = True
        else:        
            self.__solve_fixed_point()
            if compute_fic and (np.abs(self._S_E_ss - self._S_E_ss_0).max() > 0.1):
                #print("Warning: Excitatory firing-rate out of range...")
    ...
    def update_gain_parameters(self, de, di, gain_map = None, rebalance_fic = False):
        self._de = de
        self._di = di
        ...
        self.set_jacobian(compute_fic = rebalance_fic)
    ...
    def _dS(self, state):
        ...
        S_E = state[:self._nc]
        S_I = state[self._nc:]        
        I_E = self.__exc_current(S_E, S_I).clip(0, None)
        I_I = self.__inh_current(S_E, S_I).clip(0, None)
        r_E = self.phi_E(I_E)
        r_I = self.phi_I(I_I)
    ...
    def calc_hmap(self, b):
        hmap = self._raw_hmap.T.dot(b)
        self._hmap = self._normalize_map(hmap)

    def _normalize_map(self, hmap):
        #gmap_range = np.ptp(hmap)
        hmap_norm = (hmap - np.min(hmap))
        hmap_norm = hmap_norm / hmap_norm.max()
