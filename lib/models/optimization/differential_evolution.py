...
class Dev:
    ...
    def opt_func(self, theta):
        self.sample(theta)
        return self.cost_function(theta)

    def run(self, filename = None, **kwargs):
        #self.results = differential_evolution(self.opt_func, self.bounds, disp=self.verbose, polish = False, updating='deferred', workers=4, **kwargs)
        self.results = differential_evolution(self.opt_func, self.bounds, disp=self.verbose, polish = False, **kwargs)
