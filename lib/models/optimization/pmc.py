...
class Pmc(object):
    ...
    def initialize(self, model, fc=None,
        ...
        if len(t_curr.shape) == 1:
            ...
            for i in range(t_curr.shape[1]):
                ...
                for j in range(t_prev.shape[1]):
                    ...
                    [n, s] = t_curr.shape
                    #print(t_prev.shape)
                    #print(np.corrcoef(t_prev))
                    #print(tau_2)
                    #t_curr = t_curr + np.random.rand(n,t) * 1e-16
                    norm[j] = stats.multivariate_normal.pdf(t_curr[:, i], mean=t_prev[:, j], cov=tau_2, allow_singular=True)
