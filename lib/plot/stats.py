...
import pandas as pd
import scipy
from scipy.stats import ttest_ind, ranksums, spearmanr, pearsonr
import statsmodels.api as sm
#from linalg import isd
...
def lm(y, x, data = None):
    if data is None:
        ...
        # y and x should be the keys!! TODO: assert
        model = sm.OLS(data[y], sm.add_constant(data.loc[:, x], prepend=False), missing='drop')
        results = model.fit()
        beta = results.params
        pval = results.pvalues
        tval = results.tvalues
...
def regress_out(y, x, data = None):
    if data is None:
        ...
        # data is pandas dataframe, define keys
        model = sm.OLS(data[key], sm.add_constant(data.loc[:, x], prepend=False), missing='drop')
        results = model.fit()
        return results.resid
...
###############################################################################
def partial_corr(x, control, data):
    #import pdb; pdb.set_trace()
    r = np.zeros((len(x),len(data)))
    for ii, key in enumerate(x):
        #import pdb;
        #pdb.set_trace()
        model = sm.OLS(data[key], sm.add_constant(data.loc[:,control], prepend=False),missing='drop')
...
def partial_corr2(y, x, control, data):
    model = sm.OLS(data[y], sm.add_constant(data.loc[:, control], prepend=False), missing='drop')
    r0 = model.fit().resid
    r = np.zeros(len(x))
    p = np.zeros(len(x))
    for ii, key in enumerate(x):
        ...
        p[ii] = dummy[1]
    return r, p
    ## TODO output as pandas


def fisher_z(r, norm=False):
