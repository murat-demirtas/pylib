...
class Brain():
    def __init__(self, ax, surface='inflated', parc='cole', subcortex=False):
        ...
        if parc is None:
            ...
            if parc == 'cole':
                parc_file = self.template_path+'cifti/glasser_with_subcortex.dlabel.nii'
            elif parc == 'aparc':
            ...
            glass = Cifti(parc_file)
            label_info = glass.get_labels()
    ...
    def transform(self, data, affine):
        ...
        '''
        # TODO: this is done by trial and error, there should be a better way
        a = np.eye(3)
        return data*affine[0] + affine[1]*a[0] + affine[2]*a[1] + affine[3]*a[2]
    ...
    def plot_surface(self, vertex, triangle, scalar, colors, datarange, fig, hemi):
        '''
        Plot cortical surface and subcortical volume
        '''
    ...
    def plot_volume(self, scalars, colors, fig):
        '''
        Plot volume 
        '''
        # TODO: not implemented yet
        for ii, jj in enumerate(scalars):
            vol2 = copy(self.volume)
            vol2[self.volume != jj] = 0.0
            src = mlab.pipeline.scalar_field(vol2)
            blur = mlab.pipeline.user_defined(src, filter='ImageGaussianSmooth')
            mlab.pipeline.contour_surface(blur, color=tuple(colors[ii, :3] / 255.0), figure=fig)


    def add_labels(self, coords, labels):
        '''
        Add labels to specified coordinates 
        '''
        # TODO: not implemented yet
        for kk, ll in enumerate(labels):
            sf = mlab.pipeline.scalar_scatter(coords[kk])
            lbl = mlab.pipeline.labels(sf)
            lbl.mapper.label_format = ll
            lbl.property.justification = 'centered'
            lbl.property.vertical_justification = 'centered'
            lbl.visible_points.enabled = True
            lbl.visible_points.filter.filter.tolerance = 0.2



    def surface(self, hemi, view=None, dpi=100, size=None):
        '''
        Plot surface
        '''
        ...
        if hemi == 'both':
            ...
            self.plot_surface(vertex_L, self.triangles['L'], self.scalars_array['L'], self.ctx_clr['L'], self.data_range['L'], fig, 'L')
            self.plot_surface(vertex_R, self.triangles['R'], self.scalars_array['R'], self.ctx_clr['R'], self.data_range['L'], fig, 'R')
            mlab.view(self.views['L'][view][0], self.views['L'][view][1], self.views['L'][view][2])
        else:
            vertex = self.vertices[hemi]
            self.plot_surface(vertex, self.triangles[hemi], self.scalars_array[hemi], self.ctx_clr[hemi],
    ...
    def montage(self, style, subcortex=False, dpi=100):
        '''
        Create montages
        '''
        self.image = []
        if style == 'transverse':
            ...
            img_l = np.vstack((self.image[2], self.image[0]))
            img_r = np.vstack((self.image[3], self.image[1]))
    ...
    def plot(self, data, style='sides', cmap='RdBu_r',
             threshold=None, vrange=None,
             hemi_label=True, title=None,
             borders=None,
             subcortex=False, labels=None, label_orientation=None,
             dpi=100):
        '''
        Plotting script 
        '''
        ...
        img = self.montage(style, subcortex, dpi)
        self.ax.imshow(img)
        self.ax.axis('off')
