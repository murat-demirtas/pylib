...
class Bnm():
    ...
    def __init__(self, sc, gradient = None, network_mask=None, *args, **kwargs):
        ...
        if isinstance(self.sc, list):
            ...
            self.dmf = [dmf_model.Model(self.sc[ii], g=1.0, hmap=self.gradient[ii], network_mask=network_mask[ii],
                                        verbose=False, *args, **kwargs) for ii in range(2)]
        else:
            self.dmf = dmf_model.Model(self.sc, g=1.0, hmap=self.gradient, network_mask=network_mask,
